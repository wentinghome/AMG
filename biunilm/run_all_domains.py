...
from pytorch_pretrained_bert.modeling_0321_low_rouge import BertForPreTrainingLossMask
...
def main(data_dir, src_file, para_emb_folder, model_recover_path,
    ...
    # 1) ======= Test example: Now, the file is csv file (the id, keyword, paragraph, previous_paragraph) =======
    # and the pkl(the previous paragraph embedding file)
    parser.add_argument("--src_file",
                        default=src_file, type=str,
                        help="The input data file name.")
    parser.add_argument("--para_emb_folder",
                        default=para_emb_folder, type=str,
                        help="The output data file name.") # unilm-spanmlm-few-shot-plotmachine-0124
    ...
    parser.add_argument("--model_recover_path",
                        default=model_recover_path,
                        type=str,
                        required=False,
                        help="The file of fine-tuned pretraining model.") # stage-2-unilm-memspan-mlm-full-data-0124
    ...
    parser.add_argument('--max_len_key_entity', type=int, default=20,
                        help="Truncate_config: maximum length key entity.")
    parser.add_argument('--max_len_a', type=int, default=300,
    ...
    parser.add_argument('--max_position_embeddings', type=int, default=367,
    ...
    if args.do_train:
        ...
        truncate_config={'max_len_a': args.max_len_a, 'max_len_b': args.max_len_b, 'trunc_seg': args.trunc_seg,
                         'always_truncate_tail': args.always_truncate_tail}, mask_source_words=args.mask_source_words,
                          skipgram_prb=args.skipgram_prb, skipgram_size=args.skipgram_size, mask_whole_word=args.mask_whole_word,
                          mode="s2s", has_oracle=args.has_sentence_oracle, num_qkv=args.num_qkv, s2s_special_token=args.s2s_special_token,
                          s2s_add_segment=args.s2s_add_segment, s2s_share_segment=args.s2s_share_segment, pos_shift=args.pos_shift)]
        file_oracle = None
        if args.has_sentence_oracle:
            file_oracle = os.path.join(args.data_dir, 'train.oracle')
        fn_src = os.path.join( # 这种tokenized的文件是怎么处理的？
            args.data_dir, args.src_file if args.src_file else 'train.src')
        fn_para_emb = os.path.join(
            args.data_dir, args.para_emb_folder if args.para_emb_folder else 'train.tgt')
        train_dataset = seq2seq_loader.Seq2SeqDataset(
            fn_src, fn_para_emb, args.train_batch_size, data_tokenizer, args.max_seq_length,
            file_oracle=file_oracle, bi_uni_pipeline=bi_uni_pipeline) # gather
        ...
        train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=_batch_size, sampler=train_sampler,
                                                       num_workers=args.num_workers, collate_fn=seq2seq_loader.batch_list_to_batch_tensors,
                                                       pin_memory=False)
    ...
    if (recover_step is None) and (args.model_recover_path is None):
        ...
        if recover_step:
            ...
            # 加载pretrained LM weight unilm
            logger.info("***** Recover model: %s *****",
                        args.model_recover_path) # 加载预训练模型
        ...
        model = BertForPreTrainingLossMask.from_pretrained(
            ...
            attention_probs_dropout_prob=args.attention_probs_dropout_prob, num_qkv=args.num_qkv, seg_emb=args.seg_emb,
            max_len_a=args.max_len_a, max_len_key_entity=args.max_len_key_entity)# Bert model
    ...
    optimizer_grouped_parameters = [
        ...
        {'params': [p for n, p in param_optimizer if any(
            nd in n for nd in no_decay)], 'weight_decay': 0.0}# 根据是否decay进行分组
    ]
    if args.fp16:
        try:
    ...
    if args.do_train:
        ...
        for i_epoch in trange(start_epoch, int(args.num_train_epochs)+1, desc="Epoch", disable=args.local_rank not in (-1, 0)):
            ...
            for step, batch in enumerate(iter_bar):
                ...
                if args.has_sentence_oracle:
                    input_ids, segment_ids, input_mask, mask_qkv, lm_label_ids, masked_pos, masked_weights, is_next, task_idx, oracle_pos, oracle_weights, oracle_labels = batch # lm_label_ids: 被mask掉的单词的id, masked_pos:
                else:
                    # a) the original unilm information
                    input_ids, segment_ids, input_mask, mask_qkv, lm_label_ids, masked_pos, masked_weights, is_next, task_idx, \
                    input_span_id, input_memory_id, mem, mmask, prev, pmask, key_entity_tk, key_entity_mask = batch
                    memory_args = input_memory_id, mem, mmask, prev, pmask, key_entity_tk, key_entity_mask

                    # segment_ids_cpu = segment_ids.to('cpu')
                    # segment_ids_np = segment_ids_cpu.numpy()
                ...
                loss_tuple = model(input_ids, segment_ids, input_mask, lm_label_ids, is_next, masked_pos=masked_pos,
                                   ...
                                   span_attention_mask=input_span_id, memory_args=memory_args)
                masked_lm_loss, next_sentence_loss = loss_tuple
                if n_gpu > 1:    # mean() to average on multi-gpu.
                    # loss = loss.mean()
                    masked_lm_loss = masked_lm_loss.mean()
                    next_sentence_loss = next_sentence_loss.mean()
                loss = masked_lm_loss + next_sentence_loss # 为何这是两部分loss？
...
if __name__ == "__main__":
    num_train_epochs = 50 # 50 set in the paper
    split_list = ["train"]
    num_list = [50,100,200]
    domain_list = ["humans", "songs", "books"]
    domain_folder_list = ["1-wiki-human-folder", "2-wiki-song-folder", "3-wiki-books-folder"]
    mem_span_ckpt_list = ['/data0/1-wiki-human-folder/CKPT/stage-2-unilm-memspan-mlm-humans-0319/model.7.bin',
                     '/data0/2-wiki-song-folder/repository/CKPT-span5/'
                     'stage-2-unilm-memspan-mlm-songs-0306/model.10.bin',
                     '/data0/3-wiki-books-folder/repository/CKPT-span5/'
                     'stage-2-unilm-memspan-mlm-books-0306/model.10.bin']
    for domain, domain_folder, mem_span_ckpt in zip(domain_list, domain_folder_list, mem_span_ckpt_list):
        for num in num_list:
            for split in split_list:
                data_dir = '/data0/'+domain_folder+'/3_few_shot_plotmachine/example-' + str(num) + \
                '/' + split + '/'

                src_file = split + '_encoded.csv'

                para_emb_folder = '/data0/'+domain_folder+'/4_pre_entity_embed_folder/example-'+str(num)+ '/' + split + '/' +\
                                         split+'_pre_entity_embed/' + split + '_previous_entity_embd_folder'

                model_recover_path = mem_span_ckpt

                output_dir = '/data0/temp_folder_for_Table-SpanMem_fine-tune_0515/'+\
                             domain+'/' + 'example_'+ str(num) +'_saved_model'
                main(data_dir, src_file, para_emb_folder, model_recover_path,
                     output_dir, num_train_epochs)
