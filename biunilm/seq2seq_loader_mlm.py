        ...
        if file_oracle is None:
            ...
            with open(file_src, "r", encoding='utf-8') as f_src, open(file_tgt, "r", encoding='utf-8') as f_tgt:
                for src, tgt in zip(f_src, f_tgt):
                    ...
                    assert len(src_tk) > 0
                    assert len(tgt_tk) > 0
                    src_tokenized_list.append(src_tk)
                    tgt_tokenized_list.append(tgt_tk)
                    if len(self.ex_list) % 100 == 0:
                        print("num of added data: ", len(self.ex_list))
                    self.ex_list.append((src_tk, tgt_tk))

            out_src_file = file_src.split('\\')[-1] + '_tokenized.tsv'

            # write to tsv file
            with open(out_src_file, 'wt',newline='', encoding="utf8") as out_file:
                tsv_writer = csv.writer(out_file, delimiter='\t')
                for k, src_token in enumerate(src_tokenized_list):
                    src_sen = ' '.join(src_token).strip()
                    tgt_sen = ' '.join(tgt_tokenized_list[k]).strip()
                    tsv_writer.writerow([src_sen, tgt_sen])
        else:
...
class Preprocess4Seq2seqDecoder(Pipeline):
    ...
    def __call__(self, instance):
        padded_tokens_a, max_a_len = instance

        cls_tk_id = self.indexer(['[CLS]'])[0]
        sep_tk_id = self.indexer(['[SEP]'])[0]
        pad_tk_id = self.indexer(['[PAD]'])[0]

        tokens_a_index = padded_tokens_a.nonzero()
        tokens_a_id = padded_tokens_a[tokens_a_index]

        tokens_a_id = [t[0] for t in tokens_a_id.tolist()]
        # Add Special Tokens
        padded_tokens_a = [cls_tk_id] + tokens_a_id + [sep_tk_id]

        assert len(padded_tokens_a) <= max_a_len + 2

        if max_a_len + 2 > len(padded_tokens_a):
            padded_tokens_a += [pad_tk_id] * \
        ...
        if self.new_segment_ids:
            if self.mode == "s2s":
                ...
                segment_ids = [2]*max_len_in_batch # should choose this one for the context - 0117
        ...
        position_ids = []
        for i in range(len(tokens_a_id) + 2):
            position_ids.append(i)
        for i in range(len(tokens_a_id) + 2, max_a_len + 2):
            position_ids.append(0)
        for i in range(max_a_len + 2, max_len_in_batch):
            position_ids.append(i - (max_a_len + 2) + len(tokens_a_id) + 2)
        ...
        if self.mode == "s2s":
            input_mask[:, :len(tokens_a_id)+2].fill_(1)
        else:
            st, end = 0, len(tokens_a_id) + 2
            input_mask[st:end, st:end].copy_(
                self._tril_matrix[:end, :end])
            input_mask[end:, :len(tokens_a_id)+2].fill_(1)
